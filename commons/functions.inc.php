...
function get_pre_next($id=1,$class='wh_pre_next'){
    ...
    if($data_pre){
        ...
        $str .= '<a href="'.SHOP_SITE_URL.'/whshow?id='.$pre_id.'">'.$pre_desc.'</a>';
    }else{
        $str .='<a href="javascript;">没有上一篇</a>';
    }

    if($data_next){
        $next_id = $data_next['news_id'];
        $next_desc = $data_next['news_name'];
        $str .= '<a href="'.SHOP_SITE_URL.'/whshow?id='.$next_id.'">'.$next_desc.'</a>';
    }else{
        $str .='<a href="javascript;">没有下一篇</a>';
    }
